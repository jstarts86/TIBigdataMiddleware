...
es = Elasticsearch(
    [esAcc.host], http_auth=(esAcc.id, esAcc.password), scheme="https", port=esAcc.port
)
...
# debug 확인용(개발자용)
logging.basicConfig(
    filename="log_flask/" + log_filename,
    level=logging.DEBUG,
...
@app.route(
    "/preprocessing", methods=["GET", "POST"]
)  # 서버 켜져있는 방법알기위해서 GET 사용
def preprocessing():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            return "GET result"  # 지영수정

    except KeyError as e:
        err = traceback.format_exc()
        app.logger.error(
            identification
            + "request 키 에러입니다. request에 다음과 같은 키가 존재하지 않습니다. \n:"
            + str(err)
        )
        resultDic = {
            "returnCode": 400,
            "errMsg": "request 키 에러입니다. request에 다음과 같은 키가 존재하지 않습니다. \n:"
            + str(e),
        }
    ...
    ):  # 외부 해킹을 대비해 email을 mongodb에 있는 사용자인지 확인하기
    ...
    result_add_title = compound_add_text(
        email, keyword, savedDate, wordclass, stopwordTF, synonymTF, compoundTF
    )
...
@app.route("/preprocessing-text", methods=["POST"])
def preprocessing_text():
    """
    JSON Body example:
    {
      "texts": ["이것은 테스트 문서1", "이것은 테스트 문서2"],
      "wordclass": "010",
      "stopword": true,
      "synonym": true,
      "compound": true
    ...
    """
    identification = "preprocessing-text// "
...
def textmining():
    ...
    if analysisName == "count":
        ...
        result_table, result_graph, analysisInfo = word_count(
            email, keyword, savedDate, optionList, analysisName
        )
        if result_table == "failed":
            ...
            resultDic = {
                "returnCode": 200,  # 'returnDate' : datetime.datetime.now(),
                "activity": analysisName,
                "email": email,
                "keyword": keyword,
                "savedDate": savedDate,
                "analysisDate": analysisInfo["analysis_date"],
        ...
        result_table, result_graph, analysisInfo = tfidf(
            email, keyword, savedDate, optionList, analysisName
        )
        if result_table == "failed":
    ...
    # for semanticNetworkAnalysis
        ...
        result_graph, result_table, analysisInfo = semanticNetworkAnalysis(
            email, keyword, savedDate, optionList, analysisName, linkStrength
        )
    ...
    # for kmeans
        ...
        result1, result2, analysisInfo = kmeans(
            email, keyword, savedDate, optionList, analysisName
        )
        if result1 == True:
            resultDic = {
    ...
    # for ngrams
        ...
        success, result_graph, analysisInfo = ngrams(
            email, keyword, savedDate, optionList, analysisName, ngramNum, linkStrength
        )
        if success:
            resultDic = {
                "returnCode": 200,  # 'returnDate' : datetime.datetime.now(),
    ...
    # for hcluster
        ...
        success, result_graph, analysisInfo = hcluster(
            email, keyword, savedDate, optionList, analysisName, treeLevel
        )
        print("\n hcluster 분석 결과\n")
        print(result_graph)
        if success:
    ...
    # for LDA
        ...
        success, result_graph, analysisInfo = topicLDA(
            email, keyword, savedDate, optionList, analysisName
        )
    ...
    # for word embedding (Word2Vec)
        ...
        success, result_graph, analysisInfo = word2vec(
            email, keyword, savedDate, optionList, analysisName
        )
...
def wordRank():
    ...
    results = es.get(index=INDEX, doc_type="nkdb",
...
def draw():
    ...
    for i in range(0, 8):
        ...
        allDocs["query"]["bool"]["filter"][0]["range"]["post_date"]["gte"] = (
            str(startYear + (i * offset)) + "-01||/M"
        )
        allDocs["query"]["bool"]["filter"][0]["range"]["post_date"]["lte"] = (
            str(startYear + ((i + 1) * offset)) + "-01||/M"
        )
        ...
        searchDocs["query"]["bool"]["filter"][0]["range"]["post_date"]["gte"] = (
            str(startYear + (i * offset)) + "-01||/M"
        )
        searchDocs["query"]["bool"]["filter"][0]["range"]["post_date"]["lte"] = (
            str(startYear + ((i + 1) * offset)) + "-01||/M"
        )
