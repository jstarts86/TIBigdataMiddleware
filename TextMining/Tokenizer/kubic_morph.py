...
sys.path.append(
    os.path.dirname(os.path.dirname(os.path.dirname("TextMining/Tokenizer")))
)
sys.path.append(
    os.path.dirname(os.path.dirname(os.path.dirname("TextMining/Analyzer")))
)
...
def makePosList(mecab, datas, wordclass, logger, engIdxList, stopword_file):
    ...
    for i in range(len(datas["all_content"])):
        if i in engIdxList:
            posList = makePosListEN(
                datas["all_content"][i], wordclass, logger, stopword_file
            )
        else:
            posList = makePosListKR(
                mecab, datas["all_content"][i], wordclass, logger, stopword_file
            )
        result.append(posList)
    return result


def stop_syn_add_title(
    email, keyword, savedDate, mecab, wordclass, stopwordTF, synonymTF
):
    identification = (
        str(email) + "_" + "preprocessing(stop_syn)" +
        "_" + str(savedDate) + "// "
    )
    ...
    try:
        ...
        resultList = makePosList(
            mecab, datas, wordclass, logger, engIdxList, stopword_file
        )
...
def create_dir(directory, logger, identification):
    try:
        if not os.path.exists(directory):
            ...
            logger.info(
                identification + "사용자사전 폴더가 이미 있습니다. 생성하지 않습니다."
            )
        return True, None
    except Exception as e:
        err = traceback.format_exc()
    ...
    try:
        if not os.path.exists(dir + "/mecab-ko-dic-2.1.1-20180720"):
            ...
            if os.path.exists(dir) and os.path.isfile(FILE_PATH):
                ...
                logger.error(
                    identification + "사용자폴더 혹은 메캅 사용자사전 파일이 없습니다."
                )
                return (
                    False,
                    identification + "사용자폴더 혹은 메캅 사용자사전 파일이 없습니다.",
...
def compound_add_text(
    ...
    if compoundTF == True:
        ...
        compound_file = getCompound(
            "default", "", ""
        )  # 지정해놓은 default 사용자사전 적용

    if compound_file != False:
        com_df = pd.DataFrame(list(compound_file.items()),
                              columns=["단어", "품사"])
    ...
    # /usr/local/lib/mecab/dic/mecab-ko-dic을 자동으로 참조
    mecab = Mecab(
    ...
    success, doc = stop_syn_add_title(
        email, keyword, savedDate, mecab, wordclass, stopwordTF, synonymTF
    )
    ...
    try:
        ...
        err = traceback.format_exc()
        logger.info(
            identification + "return list를 만드는 중에 에러 발생 세부사항: " + str(err)
        )
    ...
    # 전체 형태소 분석한 단어들의 목록 (kubic 미리보기에 뜨도록) --> 출력 형태 변경
    return success, return_mdoc
